...
# Bcrypt configuration - we now use bcrypt directly as passlib has compatibility issues with bcrypt 5.0+
# No global context needed for direct bcrypt
...
class AuthService:
    ...
    def hash_password(password) -> str:
        ...
        # Bcrypt has a 72-byte limit. We truncate to 72 bytes to avoid errors.
        # We use bytes to ensure we don't exceed the byte limit even with multi-byte chars.
        password_bytes = pw_str.encode('utf-8')
        ...
        # Use bcrypt directly for better compatibility with 5.0+
        salt = bcrypt.gensalt()
    ...
    def verify_password(plain_password, hashed_password: str) -> bool:
        ...
        # Handle string/bytes and match the truncation used in hash_password
        if isinstance(plain_password, str):
            plain_password_bytes = plain_password.encode("utf-8")
        else:
            plain_password_bytes = plain_password
            
        safe_password = plain_password_bytes[:72]
        ...
        try:
            ...
            return bcrypt.checkpw(safe_password, hashed_password_bytes)
